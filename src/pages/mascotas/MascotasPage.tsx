...
export function MascotasPage() {
  ...
  const { data, isLoading, isError } = useMascotas(page, PAGE_SIZE, mostrarInactivas);
  const navigate = useNavigate();

  const mascotas = useMemo(() => {
    if (!data?.mascotas) return data?.mascotas;
    const termino = busqueda.trim().toLowerCase();
    if (!termino) return data.mascotas;
    return data.mascotas.filter((m) => m.nombre.toLowerCase().includes(termino));
  }, [data?.mascotas, busqueda]);
