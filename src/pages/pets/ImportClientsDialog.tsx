...
export function ImportClientsDialog() {
  ...
  const importarClientes = useImportClients();
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => (v ? setOpen(true) : handleClose())}>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Importar clientes from Excel</DialogTitle>
        </DialogHeader>
          ...
          {resultado && (
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-2 rounded-md bg-emerald-50 px-3 py-2 text-sm text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400">
                ...
                {resultado.imported} pet(s) importada(s) correctamente.
              </div>

              {resultado.errors.length > 0 && (
                <div>
                  <p className="mb-2 text-sm font-medium text-destructive">{resultado.errors.length} fila(s) con error:</p>
